...
read_value() {
    ...
    local file=$2
    # 使用 grep 和 cut 提取值
    # 查找文件中以指定键开头的行，并用 cut 命令提取等号后的部分
    value=$(grep "^${key}=" "${file}" | cut -d'=' -f2-)
    # 检查是否找到对应的键
    # 如果 VALUE 变量为空，表示未找到对应的键
    if [ -z "${value}" ]; then
        return 1
    else
        echo "${value}"
...
if [[ "$action" = "install" ]]; then
    ...
    if [[ ! -d "$data_volume/log/redis" ]]; then
        mkdir -p "$data_volume/log/redis"
    fi
    chmod -R 777 "$data_volume/log/redis"
    chmod +x ./cms.sh
    chmod +x -R ./script
    # 3.随机生成MySQL的root密码
    echo ""
    ...
    # 循环等待MySQL服务启动成功
    while
        ! docker exec -it cms-mysql sh -c 'mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" --default-character-set=utf8mb4 ccssx_boot -BN -e "select 1"' &>/dev/null
    do
        echo "Waiting for MySQL to start..."
        sleep 3
    done
    ...
    if [[ "$tenant_type" = "isp" ]]; then
        ...
        # 执行企业初始化脚本
        docker exec -it cms-mysql sh -c 'mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" --default-character-set=utf8mb4 ccssx_boot -e "source /init_tenant/isp.sql"'
        # 修改企业初始化标识
        docker exec -it cms-mysql sh -c 'mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" --default-character-set=utf8mb4 ccssx_boot -e "update cms_global_config set initialized_flag = 1"'
    elif [[ "$tenant_type" = "multi" ]]; then
        # 赋值企业初始化脚本中的变量
        docker exec -it cms-mysql sh -c 'sed -i "s|{tenant_host}|'$tenant_host'|g" /init_tenant/multi.sql'
        # 执行企业初始化脚本
        docker exec -it cms-mysql sh -c 'mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" --default-character-set=utf8mb4 ccssx_boot -e "source /init_tenant/multi.sql"'
        # 修改企业初始化标识
        docker exec -it cms-mysql sh -c 'mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" --default-character-set=utf8mb4 ccssx_boot -e "update cms_global_config set initialized_flag = 1"'
    else
    ...
    data_volume=$(read_value "VOLUME_PATH" "${old_env_path}")
    if [ $? -eq 0 ]; then
        # 输出找到的值
        echo "key 'VOLUME_PATH' is ${data_volume}"
    else
        echo "Key '${VOLUME_PATH}' not found in file '${old_env_path}'."
        exit 1
    fi
    ...
    cp -a ./cms_temp/cms.sh ./
    chmod +x ./cms.sh
    cp -a ./cms_temp/script/ ./
    chmod +x -R ./script
    ...
    $docker_compose_command up -d mysql
    # 循环等待MySQL服务启动成功
    while ! docker exec -it cms-mysql sh -c 'mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" --default-character-set=utf8mb4 ccssx_boot -BN -e "select 1"' &>/dev/null; do
        echo "Waiting for MySQL to start..."
        sleep 3
    done
